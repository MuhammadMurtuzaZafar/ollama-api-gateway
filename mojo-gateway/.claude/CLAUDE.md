...
|------|-------------|-------|------------|-------|
| Jan 13 | Baseline INT4 (v2 kernel) | 48 | 77.6μs | 4-element vectorized loads |
| Jan 13 | Phase 1 v3 (prefetch+SMEM_IDX) | 40 | 99.0μs | **Slower** - overhead too high |
| Jan 13 | Phase 1 v3 (simplified SMEM_IDX) | 49 | 75.9μs | No improvement |
| Jan 13 | Phase 2 v3 (FMA + 8-elem) | 55 | 67.1μs | 15% improvement |
| Jan 13 | Phase 2 v3 (FMA + 512 threads) | **58** | **61.4μs** | **21% improvement** |
| Jan 13 | Phase 3 INT8 embedding kernel | - | 2.26ms | **1.6x faster** logit GEMV |
| Jan 13 | Phase 4 Multirow kernel (bench) | - | 11.8μs | **2-3x faster** (kernel only) |
| Jan 13 | Stable v3 kernel | 47 | ~21μs | Coherent output, stable baseline |
| Jan 13 | v4 kernel (prefetch) | **BROKEN** | - | Garbage output despite same iteration order |
| Jan 13 | v5 kernel (__ldg() cached) | 59 | ~61μs | **Works** but no speedup on Turing |
...
|--------|----------|----------|---------|
| Attention QKV (1536x1536) | 21.1 μs | 11.8 μs | **1.8x** |
| FFN up/gate (1536x8960) | 327 μs | 105 μs | **3.1x** |
| FFN down (8960x1536) | 119 μs | 44 μs | **2.7x** |
| Vocab proj (151936x1536) | 2220 μs | 760 μs | **2.9x** |
...
**Debugging process:**
1. Tried Kahan summation → still broken (78 tok/s but garbage output)
2. Tried double precision → still broken (17 tok/s)
3. Created isolated debug test → found dequantized weights were ~42 million instead of ~0.01
...
- Logit GEMV savings: 3.63ms → 2.26ms = 1.37ms saved
...
3. 512 threads for large matrices (hidden_dim≥4096)
...
// Strategy: Dequant INT4→FP16, then use WMMA
#include <mma.h>
...
|-------|-------------|--------|
| P1 | Unrolled loads + SMEM_PAD | 58 → 90 tok/s |
| P2 | FP16 Tensor Core GEMV | 90 → 150 tok/s |
| P3 | Weight reshuffling + scale caching | 150 → 200+ tok/s |
